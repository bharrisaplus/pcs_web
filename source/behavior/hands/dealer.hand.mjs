    ...
    if ((uiPos > 0 && uiPos <= _glob.cardMax) && (ndoPos > -1 && ndoPos < _glob.cardMax)) {
      switch(true) {
        case (ndoPos < 13): { // Spades
          _name = `${_glob.cnames[ndoPos]} of ${_glob.suites[0]}`; break;
        }
        case (ndoPos < 26): { // Diamonds
          _name = `${_glob.cnames[ndoPos % _glob.cnames.length]} of ${_glob.suites[1]}`; break;
        }
        case (ndoPos < 39): { // Clubs
          let rNamePos = (ndoPos % _glob.cnames.length) * -1;

          _name = `${_glob.cnames.slice(rNamePos - 1, rNamePos)} of ${_glob.suites[2]}`; break;
        }
        case (ndoPos < 52): { // Hearts
          let rNamePos = (ndoPos % _glob.cnames.length) * -1;

          _name = `${_glob.cnames.slice(rNamePos - 1, rNamePos)} of ${_glob.suites[3]}`; break;
        }
